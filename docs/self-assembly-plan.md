...
- [x] **Phase S** — geometric: morphology end-to-end. Ideal-chain R_g²∝N (primary) + a `tests/geometric_solver.rs` morphology ladder driven from a Brownian start and read off the Phase-O observables: **chain** (p≈0.93) and **sheet** (random soup → S 0.08→0.97, one cluster, OPEN closure) genuinely self-assemble; **tube** (prolate gyration detector) and **vesicle** (closed detector) are validated on seeded near-target configs with the spontaneous-stability limitation LOGGED honestly (the isotropic-cohesion model has no bending rigidity, so a hollow tube/shell pinches/collapses — that needs the Phase-A/C amphiphile+bending follow-up). No engine change: the morphology tests are pure validation over the existing thermostat+well+patchy-director machinery.
- [~] **Phase C** — geometric: bending rigidity (`kappa_bend` κ, torque-only, emergent modulus measured in `tests/geometric_solver.rs`) + spontaneous curvature (`spont_curvature_c0` c₀) + a director→position **tilt coupling** (`tilt_coupling_strength`, a CLEAN positional potential folded into `EnergyBreakdown::tilt`, `−∇E==F` verified) that makes membrane GEOMETRY follow the normals. Outcome (C3, honest): **flat sheet is now SPONTANEOUS** from a Brownian start (ensemble mean flatness ≈0.58, S ≈0.94, one open cluster — the real bilayer upgrade from the pre-C3 nematic droplet at flatness ≈0.05–0.1). **Tube** and **vesicle** show genuine SPONTANEOUS CURVING from a flat strip/disk start (the curvature knob lifts closure far above the flat baseline while the aggregate stays one cohered cluster — no longer pinching/collapsing), validated alongside the prolate/closed detectors on seeded near-targets; FULL stable hollow-tube / closed-shell topology is NOT a reliable T=0 fixed point in this single-leaflet point model within a unit-test budget (the classic open-disk/vesicle kinetic trap) and is LOGGED honestly, not faked.
