...
impl LayoutEngine for GeometricGpuEngine {
    ...
    fn init(
        ...
        let gpu_ctx = ctx.gpu.as_ref().ok_or_else(|| "geometric-gpu requires a wgpu device".to_string())?;
        ...
        // We need class, coordination, mass, and edges with target lengths.
        let attrs = graph_shard.attributes;
        let node_class: Vec<u32> = attrs.and_then(|a| a.node_class.clone()).unwrap_or_else(|| vec![0; n]);
        let node_coord: Vec<u32> = attrs.and_then(|a| a.node_coordination.clone()).unwrap_or_else(|| vec![0; n]);
        let node_mass: Vec<f32> = attrs.and_then(|a| a.node_mass.clone()).unwrap_or_else(|| vec![1.0; n]);
        
        // Unique edges
        let mut edges_pairs: Vec<[u32; 2]> = Vec::new();
        let mut target_lens: Vec<f32> = Vec::new();
        for v in 0..n {
            ...
            for e in start..end {
                ...
                if (v as u32) < u {
                    edges_pairs.push([v as u32, u]);
                    let l = attrs.and_then(|a| a.edge_len.as_ref()).and_then(|l| l.get(e)).copied().unwrap_or(self.settings.edge_rest_len);
                    target_lens.push(l);
                }
            }
        }
        ...
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            ...
            source: wgpu::ShaderSource::Wgsl(Cow::Borrowed(include_str!("../shaders/geometric_barnes_hut.wgsl"))),
        });
        ...
        let bgl = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            ...
            entries: &[
                ...
                uniform(4), storage_ro(5), storage_ro(6), storage_ro(7),
                storage_ro(8), storage_ro(9), storage_ro(10), storage_ro(11),
            ],
        });
        ...
        let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                ...
                wgpu::BindGroupEntry { binding: 3, resource: target_lens_buf.as_entire_binding() },
        ...
        self.gpu = Some(Gpu {
            ...
            positions_buf, velocities_buf, edges_buf, target_lens_buf, params_buf,
            node_class_buf, node_coord_buf, node_mass_buf, oct_nodes_buf,
            coord_angles_buf, class_radius_buf, class_affinity_buf,
            readback_buf,
            n_nodes, n_edges, oct_capacity, positions_byte_len,
    ...
    fn step(&mut self, _ctx: &mut EngineCtx) -> StepOutput {
        ...
        let used = gpu.oct_build.rebuild(&gpu.cpu_positions, &gpu.cpu_mass, gpu.n_nodes, gpu.oct_capacity);
        if used > 0 {
            gpu.queue.write_buffer(&gpu.oct_nodes_buf, 0, bytemuck::cast_slice(&gpu.oct_build.nodes));
        }

        let params = build_params_raw(gpu.n_nodes, gpu.n_edges, &settings, used);
        gpu.queue.write_buffer(&gpu.params_buf, 0, bytemuck::bytes_of(&params));
        ...
        encoder.copy_buffer_to_buffer(&gpu.positions_buf, 0, &gpu.readback_buf, 0, gpu.positions_byte_len);
...
fn build_params_raw(n_nodes: u32, n_edges: u32, s: &GeometricSettings, n_octree: u32) -> GeometricParamsRaw {
    GeometricParamsRaw {
        n_nodes,
        n_edges,
        n_octree,
...
impl OctreeBuild {
    fn rebuild(&mut self, positions: &[f32], mass: &[f32], n_bodies: u32, capacity: u32) -> u32 {
        ...
        for i in 0..n_bodies as usize {
            for d in 0..3 {
                ...
                if p < min_pos[d] { min_pos[d] = p; }
                if p > max_pos[d] { max_pos[d] = p; }
            }
        }
    ...
    fn insert(&mut self, node_idx: usize, body_idx: u32, positions: &[f32], mass: &[f32], capacity: u32) {
        let (pos_i, mass_i) = (&positions[4 * body_idx as usize..4 * body_idx as usize + 3], mass[body_idx as usize]);
    ...
    fn get_octant(&self, pos_size: [f32; 4], pos: &[f32]) -> usize {
        let mut o = 0;
        if pos[0] >= pos_size[0] { o |= 1; }
        if pos[1] >= pos_size[1] { o |= 2; }
        if pos[2] >= pos_size[2] { o |= 4; }
        o
    }
    ...
    fn compute_com_and_rope(&mut self, idx: usize, skip: u32) -> u32 {
        ...
        for i in 0..8 {
            ...
            if child_idx == OCT_END { continue; }
            
            if first_child == OCT_END { first_child = child_idx; }
