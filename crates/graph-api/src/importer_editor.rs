...
//! package text (TOML or Nix) with the catalog's read posture (none).
...
//! as selecting a non-default source does. Every write validates first
//! (evaluating Nix through the same sandboxed evaluator as `POST /generate`),
//! then lands atomically. Runtime-added sources persist in
...
use crate::importer_package::{
    dir_is_writable, parse_http_json_package, read_package_definition, write_atomically,
    PackageFormat,
};
...
/// Validate authored package text off the async runtime (Nix evaluation is
/// synchronous and CPU-bound). A validation failure is the 400 body verbatim.
async fn validate_source_text(format: PackageFormat, source: String) -> Result<(), Response> {
    match tokio::task::spawn_blocking(move || parse_http_json_package(format, &source)).await {
        Ok(Ok(_)) => Ok(()),
...
pub async fn definition_put(
    ...
    let format = match PackageFormat::from_path(&located.path) {
        Ok(format) => format,
        Err(error) => return reject(StatusCode::BAD_REQUEST, error),
    };
    if let Err(response) = validate_source_text(format, req.source.clone()).await {
        return response;
    }
...
/// and a `.nix`/`.toml` extension.
fn validate_package_filename(name: &str) -> Result<PackageFormat, String> {
...
pub async fn importers_post(
    ...
    let format = match validate_package_filename(&req.package) {
        Ok(format) => format,
        Err(error) => return reject(StatusCode::BAD_REQUEST, error),
    };
...
mod tests {
    ...
    fn package_filenames_are_bounded_to_the_packages_dir() {
        assert_eq!(validate_package_filename("a-b_c.1.nix").unwrap(), PackageFormat::Nix);
        assert_eq!(validate_package_filename("pkg.toml").unwrap(), PackageFormat::Toml);
        for bad in ["", ".hidden.nix", "../x.nix", "a/b.nix", "Upper.nix", "x.yaml", "..", "x.nix\\"] {
