...
async fn build_alternate(
    ...
    match definition.kind {
        CatalogSourceKind::HttpJson => {
            ...
            // Nix evaluation is synchronous and uses non-Send internals;
            // keep it off the async runtime like `POST /generate` does.
            let (package, _definition) = tokio::task::spawn_blocking(move || {
                crate::importer_package::load_http_json_package(&manifest_path)
            })
            .await
