...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn reselect_rejects_stale_graph_revision_without_mutating_selection() {
    let broker = ComputeBroker::new();
    broker
        .connect_with("http://127.0.0.1:1/".to_string(), RemoteLayout::default())
        .await
        .expect("connect_with configures broker");
    ...
    let error = broker
        ...
        .await
        .expect_err("stale graph revision must reject atomically");
    assert!(error.to_string().contains("stale graph revision"));
    let selection = broker.selection_state().await;
    assert_eq!(selection.generation, 1);
    assert_eq!(selection.layout, RemoteLayout::default());
}
