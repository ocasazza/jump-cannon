...
const NIX_PACKAGE: &str = include_str!("../../../charts/jump-cannon/packages/hindsight-memory-bank.nix");

/// A packages dir holding the shipped Nix package under a catalog-declared
...
fn packages_fixture(tag: &str) -> (std::path::PathBuf, String) {
    ...
    std::fs::write(dir.join("hindsight.nix"), NIX_PACKAGE).expect("nix package");
...
/// GET exposes the authored Nix source with the read posture of the catalog
/// (no group needed); PUT carries the switch authorization, validates before
/// writing, and replaces the file atomically.
#[tokio::test]
async fn importer_definition_get_and_put_contract() {
    ...
    assert_eq!(response.status(), StatusCode::OK);
    let body = json_body(response).await;
    assert_eq!(body["format"], "nix");
    assert_eq!(body["package"], "hindsight.nix");
    assert_eq!(body["writable"], true);
    assert_eq!(body["source"].as_str().unwrap(), NIX_PACKAGE);
    ...
    let forbidden = app
        ...
        .oneshot(json_request(
            ...
            serde_json::json!({ "source": NIX_PACKAGE }),
            None,
        ))
        .await
        .unwrap();
    ...
    assert!(text_body(invalid).await.contains("missing field"));
    assert_eq!(
        std::fs::read_to_string(dir.join("hindsight.nix")).unwrap(),
        NIX_PACKAGE,
        "a rejected PUT never touches the file"
    );

    let edited = NIX_PACKAGE.replace("name = \"Hindsight memory bank\";", "name = \"Edited bank\";");
    let saved = app
        .clone()
        .oneshot(json_request(
            "PUT",
    ...
    assert_eq!(body["format"], "nix");
    assert_eq!(body["writable"], true);
    assert_eq!(std::fs::read_to_string(dir.join("hindsight.nix")).unwrap(), edited);
    assert!(
...
async fn importers_post_adds_runtime_source_and_persists_overlay() {
    ...
    escaping["package"] = serde_json::json!("../escape.nix");
    let rejected = app
        .clone()
    ...
    bad_source["source"] = serde_json::json!("throw \"nope\"");
    let rejected = app
        .clone()
        ...
        .unwrap();
    assert_eq!(rejected.status(), StatusCode::BAD_REQUEST);
    assert!(!dir.join("local-bank.nix").exists(), "rejected POST writes nothing");
    ...
    assert_eq!(item["httpJson"]["package"], "local-bank.nix");
    assert_eq!(item["httpJson"]["pollIntervalMs"], 30000);
    assert_eq!(std::fs::read_to_string(dir.join("local-bank.nix")).unwrap(), NIX_PACKAGE);
