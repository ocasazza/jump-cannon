...
| `crates/graph-vcs` | Jujutsu-inspired version-control core for graph snapshots: object-safe `VcsStore` (boxed-future convention, no async runtime), `MinigrafStore` (single-file embedded DB; in-memory on wasm32), jj-style `Commit`/`ChangeId`/snapshot merge model with first-class recorded conflicts. |
