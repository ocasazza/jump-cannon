...
- `GET /importers/sources/{id}/parameters` returns 200 JSON `{"source": "<id>", "parameters": {"<param-name>": {"label": "...", "default": "...", "values": [...], "discovered": true|false, "description": "...", "catalog": true|false}}}` enumerating the bound package's declared `parser.variables` entries — the authoritative parameter list — overlaid with the catalog's optional `parameters` UI metadata (label, discovery, static values, default). The pre-selected `default` is the catalog parameter default, else the bound instance variable, else the package default. `catalog` marks whether the catalog declared the parameter (vs. a package-only variable); the frontend applies catalog parameters explicitly and package-only variables only when changed. Discovery runs live (10 s timeout, ≤1000 items) and falls back to static `values` on error, setting `"error": "..."` when fallback is used. Returns 404 when the source is not found or 403 when runtime switching is disabled. Cached per source for 60 s (invalidated by `PUT /importers/:id/variables`).
