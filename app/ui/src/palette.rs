...
impl Section {
    ...
    fn panel(self) -> crate::Panel {
        match self {
            ...
            Section::Importers => crate::Panel::Importers,
        }
    }
...
/// Node ids are only unique within one graph session. A graph replacement or
/// server change must not serve the previous graph's cached previews.
