//! Local Web Worker client for the Generate panel's `LocalWorker` executor.
//!
//! This is the OFFLINE non-freeze path: it spawns the `tvix-worker` bundle
//! (built by trunk from `app/tvix-worker`, see `ui/index.html`) as a real
//! [`web_sys::Worker`] and hands it a Nix expression to evaluate. Because the
//! worker is a second wasm instance in a browser-owned thread, the UI thread
//! never blocks on `eval_graph` — unlike the `Inline` executor, which runs the
//! synchronous eval right on the UI thread (one 0 ms yield, then a freeze for
...
//! evals this path exists to keep off the UI thread. A persistent worker pool
//! is a possible future optimization (it would need a ready-handshake queue
//! for concurrent jobs; the Generate panel only runs one at a time).
//!
//! (Port of the retired egui renderer's `crate::worker`; the protocol —
//! READY handshake, expression in, reply JSON out — is unchanged.)

#![allow(dead_code)] // native target compiles only the stub below

#[cfg(target_arch = "wasm32")]
pub use wasm::eval_in_worker;
...
/// typecheck the Generate panel's call site.
#[cfg(not(target_arch = "wasm32"))]
pub async fn eval_in_worker(_expr: String) -> Result<tvix_wasm::GeneratedGraph, String> {
    Err("the local worker executor is wasm-only".to_string())
}
...
mod wasm {
    ...
    /// graph. The returned future resolves off the UI thread (driven by
    /// `spawn`/`spawn_local`), so the webview stays responsive while the worker
    /// evaluates.
    pub async fn eval_in_worker(expr: String) -> Result<GeneratedGraph, String> {
        ...
        // we post the expression (it would have been dropped if posted before
        // the worker's handler existed). The next message is the result JSON.
        ...
        // Await the reply. `on_message` is kept alive on the stack here until
        // the future resolves, so it is dropped (not leaked) after each eval.
    ...
    /// embedded `graph` is the canonical `toGraphJSON` wire, so it flows
    /// through the same [`tvix_wasm::parse_graph_json`] the Server backend uses
