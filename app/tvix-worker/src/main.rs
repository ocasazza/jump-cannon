...
//! off the webview's main (UI) thread.
//!
//! ## Why this exists
//!
//! `tvix_wasm::eval_graph` is a single synchronous call that can take a
//! noticeable wall-clock chunk for a large graph. Run on the UI thread it
//! freezes the webview for the duration (no paint, no input). The `Server`
...
//! that does nothing but `eval_graph`. The UI thread stays fully responsive
...
//!   so the app flows it into the SAME promotion path as a server eval),
...
//! stable URL `/tvix-worker.js` at runtime.

/// Readiness marker the worker posts once its message handler is installed.
/// Must stay in sync with the spawner's `crate::worker::READY` (app/ui).
...
mod worker {
    ...
    pub fn worker_start() {
        ...
        // dropped. The marker is shared with `crate::worker::READY` (app/ui).
...
mod tests {
    ...
    fn evaluate_ok_wraps_a_parseable_graph() {
        ...
        // The embedded `graph` must be the canonical wire the app parses
    ...
    fn evaluate_err_is_soft_json() {
        ...
        // as `{"ok":false,"error":…}` so the app can surface it inline.
